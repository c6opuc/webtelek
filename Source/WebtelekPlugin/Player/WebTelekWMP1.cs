...
{
  public class WebTelekWMP : IPlayer
  {
    ...
    public override bool Play(string strFile)
    {
      ...
      if (strFile.IndexOf("cdda:") >= 0)
      {
        ...
        _isCDA = true;
        Log.Info("WebTelekPlayer: play track:{0}/{1}", iTrack, _wmp10Player.cdromCollection.Item(0).Playlist.count);
      }
            ...
            _wmp10Player.URL.StartsWith("HTTP") || _wmp10Player.URL.StartsWith("MMS"))
      {
        ...
        {
          Log.Info("WebTelekPlayer: failed to load {0}", strFile);
          return false;
        }
      }
    ...
    {
      Log.Debug("WebTelekPlayer: protocol: {0}", _wmp10Player.network.sourceProtocol);
      Log.Debug("WebTelekPlayer: bandWidth: {0}", _wmp10Player.network.bandWidth);
      Log.Debug("WebTelekPlayer: bitRate: {0}", _wmp10Player.network.bitRate);
      Log.Debug("WebTelekPlayer: receivedPackets: {0}", _wmp10Player.network.receivedPackets);
      Log.Debug("WebTelekPlayer: receptionQuality: {0}", _wmp10Player.network.receptionQuality);
    ...
    void SongEnded(bool bManualStop)
    {
      ...
      Log.Info("WebTelekPlayer:ended {0} {1}", _currentFile, bManualStop);
      _currentFile = "";
      _isCDA = false;
