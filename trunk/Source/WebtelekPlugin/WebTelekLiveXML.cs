    ...
    public class WebTelekLiveXML 
    {
        ...
        public StringCollection getCountries()
        {
            ...
            try
            {
                ...
                expr = nav.Compile("/Rumote/channel/country");
        ...
        public StringCollection getCategories()
        {
            ...
            try
            {
                XPathNavigator nav = xml.CreateNavigator();
                XPathExpression expr;
                expr = nav.Compile("/Rumote/channel/category");
                XPathNodeIterator iterator = nav.Select(expr);
                while (iterator.MoveNext())
                {
                    XPathNavigator nav2 = iterator.Current.Clone();
        ...
        public StringCollection getChannelId()
        {
            ...
            try
            {
                XPathNavigator nav = xml.CreateNavigator();
                XPathExpression expr;
                expr = nav.Compile("/Rumote/channel/id");
                XPathNodeIterator iterator = nav.Select(expr);
                while (iterator.MoveNext())
                {
                    XPathNavigator nav2 = iterator.Current.Clone();
        ...
        public StringCollection getChannelsNames()
        {
            ...
            try
            {
                XPathNavigator nav = xml.CreateNavigator();
                XPathExpression expr;
                expr = nav.Compile("/Rumote/channel/name");
                XPathNodeIterator iterator = nav.Select(expr);
                while (iterator.MoveNext())
                {
                    XPathNavigator nav2 = iterator.Current.Clone();
        ...
        public StringCollection getURLs()
        {
            ...
            try 
            {
                ...
                {
                    XPathNavigator nav2 = iterator.Current.Clone();
                    result.Add("http://www.rumote.com/play.php?ch=" + nav2.Value);
                }
            }
            catch (Exception)
            {
        ...
        public StringCollection getSchedules()
        {
            ...
            try
            {
                ...
                {
                    string channellist = "";
                    expr = nav.Compile("/Rumote/channel[id=" + getChannelId()[i] + "]/listing/*");
                    XPathNodeIterator iterator = nav.Select(expr);
                    int j = 0;
                    int k = 0;
                    try
        ...
        {
            string tvguide = "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n<tv generator-info-name=\"generated by Rumote.com\">\n";
